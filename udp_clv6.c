...
struct in6_addr ip6_aton(const char *str){
	struct in6_addr ip6;
...
int client(const char *host, char *buf){
	...
	addr.sin6_addr = ip6_aton(host);
	//write(sock, conbuf, strlen(conbuf));
	sendto(sock, conbuf, strlen(conbuf), 0, (struct sockaddr *)&addr, sizeof(addr));
	
	int read_size;
	char rebuf[BUFLEN];
	//read_size = read_line(sock, rebuf);
	//if(read_size != 0)
	recvfrom(sock, rebuf, sizeof(rebuf), 0, NULL, NULL);
		printf("mes: %s\n", rebuf);
	subchar(buf, rebuf);
	printf("sub: %s\n", buf);
	printf("disconnect.\n");
